...
import { PROPERTY_COORDINATES, calculateDistance as calcGeoDistance } from '@/lib/properties'
  ...
  useEffect(() => {
    if (idealLocation) {
      const sorted = [...filteredProperties].sort((a, b) => {
        const distA = calcGeoDistance(idealLocation, a.city)
        const distB = calcGeoDistance(idealLocation, b.city)
        return distA - distB
      })
          ...
          <div className="grid md:grid-cols-[350px_1fr] gap-4">
            <div className="space-y-3 overflow-y-auto max-h-[calc(100vh-300px)]">
              {displayProperties.map((property) => {
                const distance = idealLocation ? calcGeoDistance(idealLocation, property.city) : null
                return (
