...
export async function POST(req: Request) {
    try {
        console.log("Ping! Request ontvangen op giveCommentSuggestion");
        ...
        console.log("Payload from ElevenLabs (giveCommentSuggestion):", body);
        ...
        if (!suggestedComment || typeof suggestedComment !== 'string' || suggestedComment.trim() === '') {
            return NextResponse.json(
                { error: 'Bad Request. Missing or invalid suggestedComment in request body.' },
                { status: 400 }
            );
        }
        ...
        const result = await giveCommentSuggestion({
            ...
            suggestedComment: suggestedComment.trim()
        });

        console.log("Result from DB (giveCommentSuggestion):", result);
        ...
        console.error('[ElevenLabs Tools Endpoint - giveCommentSuggestion] Error handling request:', error);
        return NextResponse.json(
